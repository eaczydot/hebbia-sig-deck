...
const ReferenceItem = ({ title, source, type, delay }) => (
    ...
    >
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            ...
            <span style={{ fontSize: 'clamp(12px, 1.25vw, 13px)', fontWeight: 700, letterSpacing: '0.05em', color: 'var(--color-text-secondary)' }}>{type}</span>
        </div>
        <div style={{ color: 'white', fontWeight: 500, fontSize: 'clamp(12px, 1.5vw, 14px)' }}>{title}</div>
        <div style={{ color: 'var(--color-text-tertiary)', fontSize: 'clamp(13px, 1.5vw, 14px)', fontStyle: 'italic', textAlign: 'right' }}>{source}</div>
    </motion.div>
);
    ...
    return (
        <SlideContainer>
            ...
            <div style={{ marginTop: 'var(--spacing-2xl)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 'var(--layout-gap-md)', flexWrap: 'wrap' }}>
                <div style={{ fontSize: 'clamp(13px, 1.5vw, 14px)', color: 'var(--color-text-tertiary)' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px', opacity: 0.35 }}>
                    ...
                    <div style={{ fontSize: 'clamp(12px, 1.25vw, 13px)', color: 'var(--color-text-tertiary)' }}>Deck runtime</div>
                </div>
